...
-- ============================================================
-- DONNÉES : métadonnées du module sport
-- ============================================================
...
INSERT INTO t_sport_object_i18n (object_id, lang_code, name)
SELECT o.id, 'en', 'Sport' FROM t_sport_object o JOIN t_system_module m ON o.module_id = m.id WHERE m.code = 'sport' AND o.code = 'sport'
ON CONFLICT (object_id, lang_code) DO UPDATE SET name = EXCLUDED.name;

INSERT INTO t_sport_object_i18n (object_id, lang_code, name)
SELECT o.id, 'es', 'Deporte' FROM t_sport_object o JOIN t_system_module m ON o.module_id = m.id WHERE m.code = 'sport' AND o.code = 'sport'
ON CONFLICT (object_id, lang_code) DO UPDATE SET name = EXCLUDED.name;
...
('SPORT_CODE_HELP',           'es', 'El código no se puede modificar después de la création.'),
...
('SPORT_FORCE_DELETE_CONFIRM','fr', 'Voulez-vous vraiment supprimer définitivement ce sport ?'),
('SPORT_FORCE_DELETE_CONFIRM','en', 'Are you sure you want to permanently delete this sport?'),
('SPORT_FORCE_DELETE_CONFIRM','es', '¿Está seguro de que desea eliminar permanentemente este deporte?'),
